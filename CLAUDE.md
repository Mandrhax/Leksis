...
│       ├── CaddyServiceForm.tsx         (Accès & HTTPS : 3 modes http / https (domaine) / proxy, secours HTTP, adresses de proxy, aperçu du Caddyfile — PATCH /api/admin/services)
...
- **Adresse publique détectée, plus de `NEXTAUTH_URL`** : `auth.config.ts` a `trustHost: true` (donc `AUTH_TRUST_HOST` est inutile — supprimé de `.env`) et **Auth.js réécrit l'origine de chaque requête avec `NEXTAUTH_URL`/`AUTH_URL` s'ils sont définis** (`reqWithEnvURL`) : le compose passe `NEXTAUTH_URL: ${NEXTAUTH_URL:-}` (vide par défaut) pour que l'adresse vienne des en-têtes `Host` / `X-Forwarded-*`. Les installs existantes gardent leur valeur ; `leksis config` (changement d'accès) la vide. L'ancien champ `nextauthUrl` de l'admin ne faisait rien (jamais lu) : supprimé
