...
- Le **modèle de traduction** est un choix dans une liste (`ask_translation_model` : `translategemma:27b` / `12b` / `4b`, plus le modèle actuel s'il est personnalisé) ; les modèles OCR et réécriture restent en saisie libre (`ask_model`) mais **pré-remplis avec le modèle de traduction choisi**. `OLLAMA_KEEP_ALIVE=-1` et `OLLAMA_SCHED_SPREAD=true` ne sont **plus demandés** (ni à l'installation ni dans `config`) mais restent écrits dans `.env` et surchargeables par `LEKSIS_OLLAMA_KEEP_ALIVE` / `LEKSIS_OLLAMA_SCHED_SPREAD` ; `config` les remet à ces valeurs
