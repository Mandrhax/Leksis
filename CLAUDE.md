...
- Le document traduit conserve autant que possible la structure originale (headings, tables, paragraphes) via un séparateur `|||` pour la traduction structurée par segments (`buildDocumentTranslationPrompt`, `flattenBlocks`/`applyTranslatedSegments`)
- **Fonctionnalité désactivée avec le provider `vllm`** (voir « Moteur IA » → Points d'attention) : la route garde un chemin alternatif pour les modèles à prompt délimité (TranslateGemma) — document aplati en un seul bloc de texte (`flattenBlocksPlain`), traduit en un appel via le format `<<<source>>>/<<<target>>>`, reconverti en blocks (`textToBlocks`), sans reconstruction de structure (tableaux/titres) — mais ce chemin est actuellement inatteignable en pratique puisque `isFeatureEnabled('document')` bloque la fonctionnalité en amont pour ce provider. Code conservé pour une réactivation future, pas à retirer sans discussion
...
- **Document et image désactivés avec le provider `vllm`** : `isFeatureEnabled('document' | 'image')` (`src/lib/features-guard.ts`) renvoie `false` dès que `getAiConfig().provider === 'vllm'`, quel que soit le réglage `site_settings.features.tabs` — les routes correspondantes répondent 403 et les onglets sont masqués côté workspace (`src/app/page.tsx`, calcule `enabledTabs` avec `getAiConfig()`). Raison : les modèles à prompt délimité (TranslateGemma) n'exposent pas la vision au format `image_url` générique via l'API OpenAI-compatible de vLLM (le modèle attend un format propriétaire `{type:"image", source_lang_code, target_lang_code, url}` non supporté par vLLM — erreur serveur `Failed to apply prompt replacement for mm_items['image'][0]`), et son `chat_template.jinja` packagé pour vLLM n'a aucune logique de gestion d'image. Plutôt que d'exposer une fonctionnalité qui échoue systématiquement, elle est masquée. Le toggle admin (`FeaturesForm.tsx`, section Modules actifs) reste visible mais désactivé avec une note (`t.featuresForm.disabledVllm`) quand `aiProvider === 'vllm'` (prop remontée depuis `admin/settings/page.tsx` via `getAiConfig()`) — la préférence stockée n'est pas perdue, juste ignorée tant que vLLM est actif
