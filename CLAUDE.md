...
- **`showFooterQuotes`** : booléen dans `site_settings.design.showFooterQuotes` (déplacé depuis `features` — clé lue avec repli `design.showFooterQuotes ?? features.showFooterQuotes` dans `page.tsx` pour les installs qui n'ont pas encore re-sauvegardé ce formulaire ; défaut `true` via `!== false`). Contrôle l'affichage des citations françaises en pied de page — affiché uniquement si `showFooterQuotes === true && locale === 'fr'`. Toggle dans `DesignForm` (section Footer, icône `format_quote`). Passer `showFooterQuotes` de `page.tsx` → `HomeClient` → footer guard
...
- **`site_settings.system_status`** : clé JSONB interne (hors schéma Zod de `PATCH /api/admin/settings`, jamais éditable depuis l'UI) écrite uniquement par `install.sh` (`record_backup_timestamp`, appelée à la fin de `create_backup`) via `psql -v` + `:'var'`. Contient `lastBackupAt` (ISO 8601 UTC) — la carte Backup du dashboard la lit pour afficher « il y a X » et passe au rouge au-delà de 8 jours ou en l'absence de sauvegarde
