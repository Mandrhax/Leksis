...
// Sans 'server-only' : partagé entre le serveur (lib/tones.ts, page d'accueil) et le formulaire admin.
...
// Les tons enregistrés avant l'ajout de l'italien n'ont pas de libellé `it` : on reprend celui du ton
// d'origine, sauf si l'admin a renommé le ton (le libellé anglais ne correspond plus).
function fillItalianLabel(tone: ToneConfig): ToneConfig {
  if (tone.labels?.it?.trim()) return tone
  const builtin = DEFAULT_TONES.find(d => d.id === tone.id && d.labels.en === tone.labels?.en)
  return builtin?.labels.it ? { ...tone, labels: { ...tone.labels, it: builtin.labels.it } } : tone
}

/** Tons enregistrés (ancien format migré, libellé italien d'origine rétabli) ; les défauts si la liste est vide ou invalide. */
export function normalizeTones(raw: unknown): ToneConfig[] {
  if (!Array.isArray(raw) || raw.length === 0) return DEFAULT_TONES
  return raw.map(migrateLabel).map(fillItalianLabel)
}
